...
/**
 * Canonicalises a URL before it becomes the key of a [PageRecord].
 *
 * Two URLs that differ only in tracking parameters, default port or case in
 * the host all hash to the same `urlHash`, so revisiting the same article
 * with `?utm_source=foo` and without finds one record instead of two. The
 * canonical form is also the value stored in [PageRecord.url], so a panel
 * listing "all memory for this page" sees the cleanest available URL.
 *
 * The rules, in order:
 ...
 * can still get a record (the user might want to remember it). Path and
...
object UrlCanonicalizer {
    ...
    /**
     * SHA-256 hex digest of [canonicalize]. The primary key of a [PageRecord]
